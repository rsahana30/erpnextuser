...
useEffect(() => {
  console.log("Logged in vendorCode:", vendorCode);  // 🔍 Debug line
  if (vendorCode) {
    fetchQuotationData();
  }
}, []);
      ...
      <div className="table-responsive shadow-sm rounded">
        <Table striped bordered hover responsive>
          ...
          <tbody className="text-center">
            {quotationData.length > 0 ? quotationData.map((item) => (
              <tr key={item.id}>
                ...
                <td>
                  <span className={`badge bg-${item.responseStatus === "Accepted"
                    ? "success" : item.responseStatus === "Rejected"
                      ? "danger" : "secondary"}`}>
                    {item.responseStatus || "Pending"}
                  </span>
                </td>
                <td>{item.responseDate ? formatDate(item.responseDate) : "-"}</td>
                <td>
